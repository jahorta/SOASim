...
#define DECL_OFFS(dom, NAME, R, M, B, W, ...) \
    static constexpr int32_t OFFS_##dom##_##NAME[] = { __VA_ARGS__ };
...
#undef DECL_OFFS
        ...
        #define ROW(dom, NAME, R, M, B, W, ...) \
          { AddrKey::dom##_##NAME, \
            DolphinAddr{ Region::R, AddrMode::M, static_cast<uint32_t>(B), Width::W, \
                         OFFS_##dom##_##NAME, static_cast<uint8_t>(sizeof(OFFS_##dom##_##NAME)/sizeof(int32_t)) }, \
            #dom "." #NAME },
        ADDR_TABLE_ALL(ROW)
        #undef ROW
    };
    ...
    static inline bool in_region(const MemView& v, Region r, uint32_t va) {
        switch (r) {
    ...
    bool Registry::resolve(const MemView& view, const DolphinAddr& a, uint32_t& out_va) {
    ...
    bool Registry::resolve_from_base(const MemView& view, const DolphinAddr& a, uint32_t base_override, uint32_t& out_va) {
        if (a.mode == AddrMode::Raw) {
    ...
    bool Registry::readU8(const MemView& v, AddrKey k, uint8_t& out) { uint32_t va = 0; if (!resolve(v, spec(k), va)) return false; return v.read_u8(va, out); }
    bool Registry::readU16(const MemView& v, AddrKey k, uint16_t& out) { uint32_t va = 0; if (!resolve(v, spec(k), va)) return false; return v.read_u16(va, out); }
    bool Registry::readU32(const MemView& v, AddrKey k, uint32_t& out) { uint32_t va = 0; if (!resolve(v, spec(k), va)) return false; return v.read_u32(va, out); }
    bool Registry::readU64(const MemView& v, AddrKey k, uint64_t& out) { uint32_t va = 0; if (!resolve(v, spec(k), va)) return false; return v.read_u64(va, out); }
