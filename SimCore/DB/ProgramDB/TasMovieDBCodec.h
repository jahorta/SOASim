// SimCore/DB/ProgramDB/ProgramTasMovieDB.h
#pragma once
#include "IProgramDBCodec.h"

struct ProgramTasMovieDB final : IProgramDBCodec {
    DbResult<int64_t> encode_job_into_db(DbEnv& env, int64_t job_set_id, const std::string& blueprint_ini) override;
    DbResult<PSJob>   decode_job_from_db(DbEnv& env, int64_t job_id) override;
