...
namespace simcore {
    namespace db {
        ...
        DbResult<void> BattlePlanTurnRepo::UpsertTurn(DbEnv& env, int64_t settings_id, int32_t turn_index, int32_t fake_atk_count) {
        ...
        DbResult<void> BattlePlanTurnRepo::UpsertTurnActor(DbEnv& env, int64_t settings_id, int32_t turn_index, int32_t actor_index, int64_t atom_id) {
            sqlite3* db = env.handle();
            sqlite3_stmt* st{};
            int rc = sqlite3_prepare_v2(db,
        ...
        DbResult<void> BattlePlanTurnRepo::ReplaceTurn(DbEnv& env, int64_t settings_id, int32_t turn_index, int32_t fake_atk_count, const std::vector<TurnActorBinding>& actors) {
            sqlite3* db = env.handle();
            char* err = nullptr;
            ...
            auto r1 = UpsertTurn(env, settings_id, turn_index, fake_atk_count); if (!r1.ok) { sqlite3_exec(db, "ROLLBACK;", nullptr, nullptr, nullptr); return r1; }
            sqlite3_stmt* del{};
            rc = sqlite3_prepare_v2(db, "DELETE FROM battle_plan_turn_actor WHERE settings_id=? AND turn_index=?;", -1, &del, nullptr);
            ...
            for (const auto& a : actors) {
                ...
                rc = sqlite3_step(ins);
                if (rc != SQLITE_DONE) { sqlite3_finalize(ins); sqlite3_exec(db, "ROLLBACK;", nullptr, nullptr, nullptr); return DbResult<void>::Err({ map_sqlite_err(rc),rc,"insert actors" }); }
            }
        ...
        DbResult<std::vector<BattlePlanTurnRow>> BattlePlanTurnRepo::LoadTurns(DbEnv& env, int64_t settings_id) {
            sqlite3* db = env.handle();
            sqlite3_stmt* ts{};
            int rc = sqlite3_prepare_v2(db,
            ...
            while ((rc = sqlite3_step(ts)) == SQLITE_ROW) {
                BattlePlanTurnRow r; r.settings_id = settings_id;
            ...
            while ((rc = sqlite3_step(as)) == SQLITE_ROW) {
                ...
                TurnActorBinding b; b.actor_index = sqlite3_column_int(as, 1); b.atom_id = sqlite3_column_int64(as, 2);
